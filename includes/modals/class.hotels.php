<?php
class Hotels extends DatabaseObject {

	protected static $table_name = "tbl_hotels";
	protected static $db_fields = array('id', 'slug', 'title', 'parentId', 'image', 'banner_image', 'brief', 'price_range', 'grade', 'status', 'sortorder', 'added_date', 'meta_keywords', 'meta_description');
	...
	public static function find_by_slug($slug=0){
		global $db;
		$sql = "SELECT * FROM ".self::$table_name." WHERE slug='$slug' AND parentId='0' LIMIT 1";
		$result_array = self::find_by_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
	}

	public static function find_by_cslug($slug=0){
		global $db;
		$sql = "SELECT * FROM ".self::$table_name." WHERE slug='$slug' AND parentId!='0' LIMIT 1";
		$result_array = self::find_by_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
	...
	static function find_all_byparnt($parentId=0, $limit=''){
		global $db;
		$cnod = !empty($limit)?' LIMIT '.$limit:'';
		$sql = "SELECT * FROM ".self::$table_name." WHERE parentId=$parentId $cnod  ORDER BY sortorder ASC";
		return self::find_by_sql($sql);
	}

	// homepage package list	
	public static function gethotels_limit_by($parentId=0,$limit=''){
		global $db;
		$cond = !empty($limit)?' LIMIT '.$limit:'';
		$sql = "SELECT * FROM ".self::$table_name." WHERE parentId='$parentId' AND status=1 ORDER BY sortorder ASC $cond ";
		return self::find_by_sql($sql);
	}
	...
	// view hotels Front.
	static function getHotels($limit=''){
		global $db;
		$cond = !empty($limit)?' LIMIT '.$limit :'';
	...
	// get the hotels name from it's id
	static function getHotelsName($id=0){
		global $db;
