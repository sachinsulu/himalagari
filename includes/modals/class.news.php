...
class News extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'slug', 'title', 'author', 'brief', 'content', 'status', 'meta_keywords', 'meta_description', 'news_date', 'archive_date', 'sortorder', 'type', 'display', 'linksrc', 'linktype', 'added_date', 'image', 'banner_image', 'source');
	...
	public static function get_news_by($limit=''){
		...
		$sql = "SELECT id, slug, title, news_date, image, brief, content  FROM ".self::$table_name." WHERE status=1 ORDER BY sortorder DESC $cond ";
	...
	public static function getRelatednews_by($cond='',$limit=''){
		...
		$sql = "SELECT id,title FROM ".self::$table_name." WHERE status=1 $cond1 ORDER BY sortorder DESC $cond2 ";
		return self::find_by_sql($sql);
	}
