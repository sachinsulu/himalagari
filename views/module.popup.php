...
if (!empty($popRec)) {
        ...
        <div class="modal fade" id="modal-popup-image">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
					<!--CAROUSEL CODE GOES HERE-->
                        <div id="myCarouselImg" class="carousel slide" data-interval="false">
                            <div class="carousel-inner">		
                            ';
    foreach ($popRec as $popr) {
        if (($popr->image) != "a:0:{}") {
            ...
            $active = ($count == 1) ? 'active' : '';
            if (!empty($popr->linksrc)) {
                $linkhref = ($popr->linktype == 1) ? $popr->linksrc : BASE_URL . $popr->linksrc;
                $target = ($popr->linktype == 1) ? 'target="_blank"' : '';
                $linkstart = '<a href="' . $linkhref . '" ' . $target . '>';
                $linkend = '</a>';
            }
            $restst .= '  
                <div class="carousel-item ' . $active . '">
                    ' . $linkstart . '<img src="' . $imglink . '" alt="' . $popr->title . '">' . $linkend . '
                </div>
                ';
            $count++;
        }
    ...
    if (count($popRec) > 1) {
        $restst .= '
                        ...
                        <!--Begin Previous and Next buttons-->
                        <a class="carousel-control-prev" href="#myCarouselImg" role="button" data-slide="prev">
                            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                            <span class="sr-only">Previous</span>
                        </a>
                        <a class="carousel-control-next" href="#myCarouselImg" role="button" data-slide="next">
                            <span class="carousel-control-next-icon" aria-hidden="true"></span>
                            <span class="sr-only">Next</span>
                        </a>
        ';
